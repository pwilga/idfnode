...
void app_main(void) {
  ...
  xTaskCreate(heartbeatTask, "heartbeatTask", 4096, NULL, 0,
              &heartbeatTaskHandle);

  // xTaskCreate(printSysInfoTask, "printSysInfoTask", 4096, NULL, 0,
  // &printSysInfoTaskHandle);

  full_nvs_flash_init();
  wifi_sta_init();

  xTaskCreate(tcp_server_task, "tcp_server", 8192, NULL, 0, NULL);
  xTaskCreate(led_blink_task, "ledBlinkTask", 2048, NULL, 0,
              &ledBlinkTaskHandle);
