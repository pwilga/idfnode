...
#include "esp_ota_ops.h"   // OTA API: esp_ota_begin, write, end, set_boot_partition
#include "esp_partition.h" // esp_ota_get_next_update_partition()
...
static void handle_ota(const int client_sock)
{
    ...
    while (read_bytes)
    {
        ...
        if (err != ESP_OK)
        {
            ESP_LOGE(TAG, "Failed to write OTA data chunk (%d bytes): %s", read_bytes, esp_err_to_name(err));
