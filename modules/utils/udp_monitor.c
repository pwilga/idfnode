...
#define MONITOR_PORT 5005

static int udp_sock = -1;
static struct sockaddr_in client_addr;
static bool client_connected = false;

static void udp_monitor_task(void *arg)
{
    ...
    ESP_LOGI(TAG, "Waiting for UDP client on port %d...", MONITOR_PORT);
    ...
    while (1)
    {
        ...
        if (client_connected &&
            (esp_timer_get_time() - last_seen_us > timeout_sec * 1000000LL))
        {
